...
{	
	...
	if(dir != NULL)
	{
		...
		while((ptr = readdir(dir)) != NULL)
		{
			if(strcmp(ptr->d_name, "index.txt") == 0)
			{
				...
				sprintf(tmp, "%s/%s", pathToLanguageDirectory, ptr->d_name);
				parseIndexFile(tmp);
				break;
			}		
		}
		...
		if(_languageNum > 0)
		{
			...
			while((ptr = readdir(dir)) != NULL)
			{
				if((strstr(ptr->d_name, ".txt") > 0)&&(!(strcmp(ptr->d_name, "index.txt") == 0)))
				{
					memset(tmp, 0, sizeof(tmp));
					sprintf(tmp, "%s/%s", pathToLanguageDirectory, ptr->d_name);
	...
	for(i = 0; i < _languageNum; i++)
	{
		//printf("--language: %s, %d\n", _ppLanguageNameS[i], _languageNum);
	...
	if(i < _languageNum)
	{
		...
		{
			printf("_item name: %s, content: %s\n",_ppItems[i]->name(), _ppItems[i]->getContent());
		}*/
	}	
}
		...
		for(int i = 0; i < _itemNum; i ++)
		{
			...
			if(_ppItems[i]->name() != NULL)
			{
				...
				if(_ppItems[i]->getContent() != NULL)
					pFile->write((BYTE*)_ppItems[i]->getContent(), strlen(_ppItems[i]->getContent()));
				pFile->write((BYTE*)"\r\n", 2);
			}
		}
		delete pFile;
...
void CLanguageLoader::parseIndexFile
	(char* path)
{
		...
		while(1)
		{
			...
			if(lineOffset > 0)
			{
				if(gotHead)
				{
					...
					{
						printf("too long language name than defined LanguageNameLength(%d)\n", LanguageNameLength);
					}
					else
					{
						...
						{
							printf("too much language > defined MaxLanguageSupported(%d) listed in index.txt\n", MaxLanguageSupported);
							break;
						}
					}					
				}
...
int CLanguageLoader::GetLineEnd(char* buffer, int bufferLen)
{
	...
	else
	{
		...
		printf("-- read line bigger than buffer: %d", bufferLen);
		return -1;
	}
}

int CLanguageLoader::LanguageOfTxtFile
	(char* path)
{
 	//printf("pars file : %s\n", path);
	char readBuffer[maxLineBufferLength + 4];
...
void CLanguageLoader::LoadLanguage(char* language)
{
	...
	char readBuffer[maxLineBufferLength + 4];
	memset(readBuffer + maxLineBufferLength, 0, 4);
	printf("--open language file: %s\n", _ppTxtfilePath[_currentLanguage]);
...
CLanguageItem* CLanguageLoader::createItem
	(char* name, char* content)
{
	...
	{
		printf("--add too much language item > maxLanguageItems(%d) defined\n", maxLanguageItems);
		return NULL;
	}
	return NULL;
}
