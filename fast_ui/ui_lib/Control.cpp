...
void CStaticText::DrawText(CFont* pFont)
{
	...
	//	printf("--CStaticText::Draw leftTop:%d, %d\n",tplft.x, tplft.y);
    CSize txCSize;
	...
	//printf("--txt : %s, x:%d, y:%d, w:%d, h:%d\n", txt, tplft.x, tplft.y, txCSize.width, txCSize.height);
...
{
	int i = _currentHilight;
	int controlNum = GetChildrenCount();
...
void CPosiBarV::Draw(bool bHilight)
{
	...
	//printf("CPosiBarV::Draw: %d, %d\n", _wholeLen, _currentPosi);
	pBrushB->Draw(this);
...
bool CBtn::OnEvent(CEvent *event)
{
	...
	if(event->_type == eventType_key)
	{
		switch(event->_event)
		{
			case key_right:
			case key_ok:
				printf("--on button : 0x%x\n", this->GetOwner()->GetApp());
