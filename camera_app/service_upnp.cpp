...
void getStoragePropertyName(char *prop, int len) {
    if (!prop) {
        return;
    }

    memset(prop, 0x00, len);
    char tmp[256];
    memset(tmp, 0x00, sizeof(tmp));
    agcmd_property_get(PropName_Pattition, tmp, "");
    snprintf(prop, len, "temp.%s.status", tmp);
}
...
public:
    ...
    virtual void EventPropertyUpdate(changed_property_type state) {
        if (property_type_storage == state) {
            ...
            CAMERALOG("%s = %s", storage_prop, tmp);
...
static void signal_handler_SIGTERM(int sig)
{
    ...
    memset(systemCMD, 0x00, sizeof(systemCMD));
