...
int StandardTcpSession::SendBack(char* data, int len)
{
    ...
    //{
        ...
        if (rt < 0) {
            CAMERALOG("send() failed with ret %d", rt);
            return rt;
        }
        if (len > (int)sizeof(head.data)) {
            int ret = 0;
            int sent = 0;
            const int LEN_4K = 4096;
            do {
                int tosend = ((head.appendLength - sent) > LEN_4K) ? LEN_4K : (head.appendLength - sent);
                ret = send(_fd, data + sizeof(head.data) + sent, tosend, MSG_NOSIGNAL);
                //CAMERALOG("tosend = %d, ret = %d", tosend, ret);
                if (ret >= 0) {
                    sent += ret;
                } else {
                    CAMERALOG("send() failed with ret %d", ret);
                    rt = ret;
                    break;
                }
            } while (sent < head.appendLength);
            if (rt >= 0) {
                rt += sent;
            }
        }
