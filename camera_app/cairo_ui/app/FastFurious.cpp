...
#define BEFORE_SECONDS_SHORT (3)
#define AFTER_SECONDS_LONG   (5)
#define AFTER_SECONDS_SHORT  (3)
...
int changeSensorMode(int opr_mode)
{
    if ((opr_mode != Sensor_WORK_MODE_NORMAL) &&
        (opr_mode != Sensor_WORK_MODE_PERFORMANCE))
    {
        CAMERALOG("Invalid sensor mode %d.", opr_mode);
        return -1;
    }

    int ret_val = 0;
    int dev_num = -1;
    const char agsys_iio_dir[] = "/sys/bus/iio/devices/";
    const char agsys_iio_sub_dir[] = "iio:device";
    char agsys_iio_bno055_name[32] = "bno055";
    char opr_mode_path[256];

    dev_num = agsys_iio_find_type_by_name(agsys_iio_bno055_name,
        agsys_iio_dir, agsys_iio_sub_dir);
    if (dev_num >= 0) {
        //CAMERALOG("Found IIO device: %s @ %s%s%d",
        //    agsys_iio_bno055_name,
        //    agsys_iio_dir, agsys_iio_sub_dir, dev_num);
    } else {
        CAMERALOG("agsys_iio_find_type_by_name() failed with ret %d.", dev_num);
        ret_val = dev_num;
        goto agboard_helper_insmod_bno055_exit;
    }
    snprintf(opr_mode_path, sizeof(opr_mode_path), "%s%s%d/in_mode",
        agsys_iio_dir, agsys_iio_sub_dir, dev_num);
    ret_val = aglib_write_file_int(opr_mode_path, &opr_mode);
    //CAMERALOG("aglib_write_file_int(%s, 0x%04X) = %d.",
    //    opr_mode_path, opr_mode, ret_val);

agboard_helper_insmod_bno055_exit:
    return ret_val;
}
...
int get060AfterLength() {
    char tmp[256];
    memset(tmp, 0x00, 256);
    agcmd_property_get(PropName_060_Length_After, tmp, "3");
    ...
    , _gpsReliableIndex(GPS_Reliable_Index_Loose)
{
    ...
    _pIcnFeedback = new BmpImage(
        _pPanel,
        Point(160, 96), Size(80, 80),
        "/usr/local/share/ui/BMP/dash_menu/",
        "feedback-bad.bmp");
    _pIcnFeedback->SetHiden();

    _pStart = new Button(_pPanel, Point(40, 170), Size(320, 60), Color_Black, Color_Grey_1);
    _pStart->SetStyle(30);
    _pStart->SetBorder(true, Color_White, 4);
    _pStart->SetText((UINT8 *)"Start Countdown", 32, Color_White, FONT_ROBOTOMONO_Bold);
...
void CountdownTestWnd::_addClipSceneData()
{
    ...
    if (_bExceed50kmh && _bExceed100kmh) {
        ...
        int after_sec = get060AfterLength();
        if (after_sec < AFTER_SECONDS_LONG) {
            after_sec = AFTER_SECONDS_LONG;
        }
        ...
        int after_sec = get060AfterLength();
        if (after_sec < AFTER_SECONDS_SHORT) {
            after_sec = AFTER_SECONDS_SHORT;
        }
        _cp->MarkVideo(time_past_ms / 1000 + BEFORE_SECONDS,
            after_sec, (const char*)&desc, sizeof(desc));
    }
}
...
bool CountdownTestWnd::_isMoveBegin(GPSSpeed &speed)
{
    ...
    const float Min_Moving_Speed = 2.0;
        ...
        (speed.speedkmh > _historySpeed[0].speedkmh))
    {
        ...
        for (i = 0; i < CD_HISTORY_RECORD_NUM - 1; i++) {
            if ((_historySpeed[i].speedkmh <= Min_Moving_Speed) ||
                (_historySpeed[i + 1].speedkmh <= Min_Moving_Speed))
            {
                index_speed = i;
                break;
...
#if 0
    if (((_bestSpeed > 0.0) && (speed.speedkmh + 16.0 < _bestSpeed))
        || (speed.speedkmh + 10.0 < _historySpeed[0].speedkmh))
    {
        if (_bExceed50kmh) {
...
void AutoTestWnd::_addClipSceneData()
{
    ...
    if (_bExceed50kmh && _bExceed100kmh) {
        ...
        _cp->MarkVideo(desc.usec_offset_100kmh / SEC_TO_USEC + BEFORE_SECONDS_SHORT,
            after_sec,
            (const char*)&desc, sizeof(desc));
    }else if (_bExceed50kmh) {
        // mark the test, add clip description for it:
        ...
        _cp->MarkVideo(time_past_ms / 1000 + BEFORE_SECONDS_SHORT,
            after_sec, (const char*)&desc, sizeof(desc));
    }
}
...
void ScoreFragment::_storeFakeScores()
{
    ...
    if (_bImperialUnits) {
        ...
        d1.AddMember("50kmh", array_30mph, allocator);
    }
    ...
    if (_bImperialUnits) {
        ...
        d1.AddMember("100kmh", array_60mph, allocator);
    }
...
void ScoreFragment::_showScoreScroller()
{
    ...
    _pScore_1->SetStyle(font_size, FONT_ROBOTOMONO_Medium, Color_Grey_2, CENTER, MIDDLE);
    if (_bImperialUnits) {
    ...
    _pScore_2->SetStyle(font_size, FONT_ROBOTOMONO_Medium, Color_Grey_2, CENTER, MIDDLE);
    if (_bImperialUnits) {
    ...
    , _cp(cp)
{
    ...
    _pViewPager = new ViewPager("TestScoreViewPager",
        _pPanel, this, Point(0, 0), Size(400, 400), 4);
