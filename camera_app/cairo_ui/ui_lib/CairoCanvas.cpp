...
int CairoCanvas::GetTextLenInPixel(const char *pTxt, FONT fonttype, int txtSize)
{
            ...
            for (int i = 0; (i < l) && (lines_drawn < lines); i++) {
                ...
                switch (h_align) {
                    case LEFT:
                        ...
                        break;
                    case CENTER:
                        pos_x = leftTop.x + (areaSize.width - te.x_advance) / 2;
                        break;
                    case RIGHT:
                        pos_x = leftTop.x + (areaSize.width - te.x_advance);
                        break;
                    default:
                        break;
                }
                ...
                //}
                cairo_move_to(_cr,
                    pos_x/* + space_num * character_width*/,
    ...
    CBmp* bmp)
{
    ...
    if (bmp->getHeight() <= 0) {
        CAMERALOG("Invalid image height %d!", bmp->getHeight());
        return;
    }
