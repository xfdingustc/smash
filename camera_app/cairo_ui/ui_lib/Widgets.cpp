...
Point StaticText::GetTxtEndPos()
{
    CairoCanvas* pCav = this->GetCanvas();
    int len = pCav->GetTextLenInPixel((const char *)_pText, (FONT)_fonttype, _txtSize);
    Point endpos = GetAbsPos();
    Size size = GetSize();
    switch (_hAlign) {
        case LEFT:
            endpos.x = endpos.x + len;
            break;
        case CENTER:
            endpos.x = endpos.x + size.width / 2 + len / 2;
            break;
        case RIGHT:
            endpos.x = endpos.x + size.width;
            break;
    }
    return endpos;
}
            ...
            for (int j = 1, l = 0; j < _code_width - 1; j++) {
                ...
                if ((_qrcode->data[(i-1)*(_code_width - 2) + (j - 1)] & 0x01)) {
                    for (int s = 0; s < rows; s++) {
                        ...
                        memset((char*)&(_bmpBuf[pp]), 0x00, sizeof(COLOR)*bits);
                    }
                }
                if (bits == m + 1) {
                    l++;
    ...
    , _moveBase(0)
{
    ...
    _pScrollerCanvas = new CairoCanvas(
        canvasSize.width, canvasSize.height,
        bitdepth);
    _scrollableSize = _pScrollerCanvas->GetSize();
...
void Scroller::setScrollableSize(const Size& size)
{
    _scrollableSize = size;
}
...
void Scroller::Draw(bool bPost)
{
    ...
    if (_viewPortLft.y > 0) {
        _pScrollerCanvas->DrawRectGradient(
            Point(0, _viewPortLft.y),
            Size(_scrollableSize.width, 20),
            _colorBG, 0.5,
            _colorBG, 0.0
            );
    }
    if (_viewPortLft.y + _viewPortSize.height < _scrollableSize.height) {
        _pScrollerCanvas->DrawRectGradient(
            ...
            case TouchEvent_Move_Changed:
                if (_bTriggered) {
                    ...
                    if (movement > Min_move) {
                        ...
                        {
                            _viewPortLft.y = 0;
                        } else if (_viewPortLft.y + _viewPortSize.height > _scrollableSize.height) {
                            _viewPortLft.y = _scrollableSize.height - _viewPortSize.height;
                        }
                        _moveBase = y_move;
                        Draw(true);
                    }
...
ViewPager::ViewPager(const char *name,
        Container* pParent,
        Window* pOwner,
        const Point& leftTop,
        const Size& size,
            ...
            case eventType_touch:
                switch (event->_event)
                {
                    case TouchEvent_Move_Ended: {
                        ...
                        if ((flick == TouchFlick_Left) ||
                            (flick == TouchFlick_Right))
                        {
                            if (!_duringAnim) {
                                _flick = flick;
                                _duringAnim = true;
