    ...
    :_itemNum(0)
{
    ...
    for (int i = 0; i < MaxLanguageSupported; i ++) {
        ...
        memset(_ppLanguageNameS[i], 0x00, LanguageNameLength);
    }
    ...
    if (dir != NULL) {
        ...
        if (_languageNum > 0) {
            ...
            for (i = 0; i < _languageNum; i++) {
                ...
                memset(_ppTxtfilePath[i], 0x00, 256);
            }
            while ((ptr = readdir(dir)) != NULL) {
...
CLanguageLoader::~CLanguageLoader()
{
    ...
    char dirname[256];
    snprintf(dirname, sizeof(dirname), "%s/allString.txxt", tmp);
    CFile *pFile = new CFile(dirname, CFile::FILE_WRITE);
    if (pFile) {
...
void CLanguageLoader::parseIndexFile(char* path)
{
    ...
    memset(readBuffer + maxLineBufferLength, 0x00, 4);
...
int CLanguageLoader::GetLineEnd(char* buffer, int bufferLen)
{
    ...
    if (offset > 0)
    {
        memset(offset, 0x00, bufferLen + buffer - offset);
    ...
    ,_action(NULL)
{
    if ((name != NULL)&&(strlen(name) > 0))
    {
        ...
        memset(_name, 0x00, strlen(name) + 1);
        strcpy(_name, name);
    }
    ...
    if ((content != NULL)&&(strlen(content) > 0))
    {
        ...
        memset(_content, 0x00, strlen(content) + 1);
