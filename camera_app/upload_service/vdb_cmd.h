...
typedef struct vdb_space_info_s {
	...
	uint32_t used_space_lo;		// used space in bytes (including marked)
	uint32_t used_space_hi;
	uint32_t marked_space_lo;	// marked space in bytes
...
//
//	VDB_CMD_SetClipAttr
//		e.g: attr_mask = CLIP_ATTR_RO, attr_value = CLIP_ATTR_RO will
//		set CLIP_ATTR_RO
//
//-----------------------------------------------------------------------
...
// clip flags: vdb_clip_info_ex_s.inherited.flags
...
#define CLIP_ATTR_RO		(1 << 1)	// readonly, TBD
#define CLIP_ATTR_AUTO		(1 << 2)	// auto generated, TBD
#define CLIP_ATTR_UPLOADED	(1 << 3)	// clip has been uploaded
#define CLIP_ATTR_LIVE_MARK	(1 << 4)	// avf_camera_mark_live_clip(), live_mark_flag

#define _CLIP_ATTR_READONLY	(CLIP_ATTR_LIVE | CLIP_ATTR_LIVE_MARK)
