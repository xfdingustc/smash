...
#define act_on_error(x, s, a)                                                               \
    ...
    if ((x) == -1) {                                                                        \
        ...
        if (__act_errno != 0) {                                                             \
            if (g_pDbgFile != NULL) fprintf(g_pDbgFile, "%s:%d: ERROR(%d): %s\n", "client.c", __LINE__, __act_errno, __act_error);		\
        }                                                                                   \
        a;                                                                                  \
    } else if (TEST_DEBUG) {                                                                 \
...
#define log1(sock_logr, s, a1)                                                              \
	do {                                                                                    \
		...
		char __log_buf[2048];                                                               \
		ftime(&__log_buft);                                                                 \
		snprintf(__log_buf, 2048, "%ld.%03d:%s:%d:" s "\n", __log_buft.time, __log_buft.millitm, "client.c", __LINE__, (a1));	            \
		write2((sock_logr), __log_buf, (INT32) strlen(__log_buf));			                                    \
		if (TEST_DEBUG && g_pDbgFile != NULL) fprintf(g_pDbgFile, "LOGGED: %s", __log_buf);	\
	} while(false)
#endif
...
#define log_on_Xwrite_error(sock, buf, size, s, g)      \
    do {                                                \
        ...
        act_on_error((size) == write2((sock), (buf), (size)) ? 0 : -1, (s), close2((sock)); log1(sock_logr, "%s ... ERROR", s); goto g);                        \
    } while(false)
