...
public:
	...
	struct ThreadFuncPara
	{
		CThread* self;
		void* para;
		ThreadFuncPara(CThread* p,void* pp): self(p), para(pp){};
	};
	CThread(const char *pName = "no name thread", PRIORITY priority = NORMAL, unsigned int statckSize = 0, void* pParam = NULL);
	virtual ~CThread();
	void SetThreadMain(RUN_FUN_PTR p){	_pMainFunc = p;};
	void Go();
	void Pause();
	virtual void Stop();
	void Finish();
...
private:
	...
	RUN_FUN_PTR 		_pMainFunc;
	pthread_t   			_hThread;
	ThreadFuncPara*	 	_pPara;
	int 				_bRuning;
	char				_name[40];
...
public:
	...
	CFile(char* path, File_Open_Mode mode);
	~CFile();
