...
public:
	...
	int SendBack(char* data, int len) {_sessionService->sendSessionBack(_sendBack, data, len);};
private:
...
public:
	...
	int ProecessEvelope(StringEnvelope* pEv,  SessionResponseSendBack* sendback);
	int ProcessCmd();
	virtual void Stop();
