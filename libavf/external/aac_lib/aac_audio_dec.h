...
typedef struct au_aacdec_config_s {
	enum audio_fs sample_freq;		// sample frequency
	...
	enum audio_fs fs_out;				// sample frequency of bit stream
	enum audio_fs fs_core;			// core sample frequency for AAC+ bit stream
	...
	u8	* codec_lib_backup_addr;	// address of library backup memory
	u16	codec_lib_backup_size;		// size of library backup memory
	u8	need_more;					//
