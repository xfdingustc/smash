    ...
    : _size(width, height)
{
    ...
    //printf("%s() line %d create Canvas: width = %d, height = %d, stride = %d\n",
    ...
    // TODO:
    FT_Library *_ft_library = FreetypeFont::getInstance()->getFTLib();
    FT_Face *_ft_face = FreetypeFont::getInstance()->getFTFace();
    ...
    _font_face = cairo_ft_font_face_create_for_ft_face(*_ft_face, 0);
    cairo_status_t status = cairo_font_face_set_user_data(_font_face, &_cr_key,
                               _ft_face, (cairo_destroy_func_t)FT_Done_Face);
    if (status) {
    ...
    cairo_set_font_face(_cr, _font_face);
...
void CairoCanvas::DrawRect(
    ...
    COLOR color, double radius, float alpha)
{
    ...
    COLOR c2)
{
    ...
    if (!pat) {
        printf("create linear pattern failed\n");
        return;
    }
...
void CairoCanvas::DrawCap(
    ...
    COLOR color)
{
    //printf("%s() line %d ########## radius = %d, angle_A = %f, angle_B = %f\n",
    //    __FUNCTION__, __LINE__, radius, angle_A, angle_B);
    ...
    int h_align, int v_align)
{
    ...
    int g = (color & 0x07E0) >> 5;
    int r = color & 0x001F;
    cairo_set_source_rgb(_cr, ((float)b) / 31, ((float)g) / 63, ((float)r) / 31);
...
#if 0
    printf("%s() line %d, fe.ascent = %f, fe.descent = %f, "
        "fe.height = %f, "
    ...
    printf("%s() line %d, pTxt = %s, txtSize = %d, "
    ...
    do {
        ...
        if (strlen(txt) == 0) {
            ...
            cairo_text_extents(_cr, txt, &te);
            UINT16 txt_width = te.x_advance;
            int character_width = txt_width / strlen(txt);
            if (character_width <= 0) {
                break;
            }
            int num = areaSize.width / character_width;
        ...
        case BmpFillMode_repeat:
            if (bmp->getColorDepth() == 16)
            {
                for (int i = lefttop.y; i < lefttop.y + size.height; i++)
                {
                    ...
                    {
                        memcpy(_pBuffer + i * _size.width + lefttop.x + j * bmp->getWidth(),
                            line, size.width * 2);
                    }
