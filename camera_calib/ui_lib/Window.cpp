...
void Window::willShow()
{

}

void Window::willHide()
{

}

void Window::Show(bool bPost)
{
    this->willShow();
    _bVisible = true;
    if (this->GetMainObject()) {
        this->GetMainObject()->Draw(bPost);
    }
}
...
void Window::setVisibleArea(const Point& leftTop, const Size& size)
{
    ...
    {
        //printf("%s() line %d window %s become invisble\n",
        //    __FUNCTION__, __LINE__, getWndName());
        _bVisible = false;
        Hide();
        _leftTop.x = 0;
        _leftTop.y = 0;
