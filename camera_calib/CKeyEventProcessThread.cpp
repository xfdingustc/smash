...
{
    if (_devs[0]._fd < 0) {
        printf("_devs[0]._fd < 0, cant start Event Process Thread normally!\n");
        return;
    }
    ...
    while (_bExecuting) {
        ...
        {
            case -1:
                printf("select error in EventProcessThread\n");
                break;
            case 0:
                break;
            default:
    ...
    , _addedNum(0)
{
    ...
    if (ret_val) {
        printf("%s(): get input list with error %d",
            __func__, ret_val);
        return;
    }
    ...
    for (int i = 0; i < event_list.valid_num; i++) {
        ...
        if (_devs[i]._fd < 0) {
            printf("open %s error", _devs[i]._name);
            return;
        }
        _numDevs++;
    }
    ...
    if (pwait_info1 == NULL) {
        printf("agcmd_property_wait_start PropName_Input_Device failed\n");
    }
}
...
{
    printf("input device changed Enter\n");

    //_mutex->Take();
    _bDeviceChanged = true;
    printf("input device changed\n");
    //_mutex->Give();
}
