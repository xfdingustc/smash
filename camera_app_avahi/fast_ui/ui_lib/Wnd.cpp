...
*   
*/ 
CWnd::CWnd(const char *name, CWndManager* pManag) 
    :_pMainObject(NULL)
    ,_pWndManager(pManag)
    ,_name(name)
    , _bShown(false)
...
bool CWnd::OnEvent(CEvent* event)
{
	...
	//CAMERALOG("--OnEvent of %s end", _name);
	return rt;
}
...
void CWnd::Show(bool bResetFocus)
{
	...
	{	
		this->willShow();
		_bShown = true;
		//CAMERALOG("CWnd::Show 0x%x", this->GetMainObject());
...
void CWndManager::Destroy()
{
	if(_pInstance != NULL)
	{
		CAMERALOG("delete ui CWndManager 0");
		delete _pInstance;
	}
}
...
CWndManager::~CWndManager()
{
	CAMERALOG("--~CWndManager 1");
	for(int i = 0; i < _number; i++)
	{
		if(_ppWndList[i] != 0)
		{
			CAMERALOG("-- delete wnd: %s", _ppWndList[i]->getName());
			delete _ppWndList[i];
		}
	}	
	delete _lock;
    CAMERALOG("--~CWndManager destroyed");
}
