...
*   
*/
ui::CBitRecWnd::CBitRecWnd(CWndManager *pMgr, CCameraProperties* cp)
...
bool ui::CBitRecWnd::OnEvent(CEvent *event)
{
    ...
    switch (event->_type)
    {
        ...
        case eventType_key:
            if (_bScreenSaver &&
                // Only press the 4 keys on camera can exit screen saver:
                ((event->_event == key_right_onKeyUp) ||
                (event->_event == key_ok_onKeyUp) ||
                (event->_event == key_up_onKeyUp) ||
                (event->_event == key_down_onKeyUp)))
            {
                _bScreenSaver = false;
                _screenSaverCount = screenSaveTime;
                ScreenSave(false);
...
ui::CBitInforWnd::CBitInforWnd(CWndManager *pMgr, CCameraProperties* cp)
...
{
    //CAMERALOG("--event: %d", event->_event);
    bool b = true;
    switch (event->_type)
    {
        case eventType_internal:
            if ((event->_event == InternalEvent_app_timer_update)) {
                if (_ppLinkSymble[_currentInfor] != NULL) {
                    //CAMERALOG("-- timer update :%d", _currentInfor);
...
void ui::CBitInforWnd::SetSymble	(CSymbleWithTxt* p, int index)
{
...
{
    int i = -1;
    //CAMERALOG("updata: 0x%x", _ppLinkSymble[_currentInfor]);
    if (_ppLinkSymble[_currentInfor]!=NULL) {
        i = _ppLinkSymble[_currentInfor]->getCurrentSymble();
        _pBitInforSymble->SetCurrentSymbleBmp(
            _ppLinkSymble[_currentInfor]->getCurrentSymbleBmp());
    } else {
    ...
    switch (_currentInfor)
    {
        case infor_language:
            if (_pLanCtrlWnd == NULL) {
                //CAMERALOG("-- %x",  _ppLinkSymble[infor_language]);
...
void ui::CBitQrCodeControl::Draw(bool bHilight)
{
    //if(_bmpBuf != 0)
    {
        ...
        if (pCav->isBitCanvas()) {
            ...
            int r = X % 8;
            //CAMERALOG("--x: %d, r: %d",X, r);
            if (bDoubleSize) {
                //CAMERALOG("--double size");
                UINT8* p1, *p2, *p3, *p4;
                UINT8 s1, s2;
...
*   
*/
ui::CBitQRCodeWnd::CBitQRCodeWnd(CWndManager *pMgr, CCameraProperties* cp)
    :inherited("BitRecWnd", pMgr)
    ,_cp(cp)
{
    _pPannel = new CPanel(NULL, this, CPoint(0, 0), CSize(128, 64), 10);
...
*   
*/
ui::CBitPowerOffPreapreWnd::CBitPowerOffPreapreWnd(CWndManager *pMgr, CCameraProperties* cp)
    :inherited("BitRecWnd", pMgr)
    ,_cp(cp)
    ,_counter(3)
{
...
*   
*/
ui::CWifiControWnd::CWifiControWnd(CWndManager *pMgr, CCameraProperties* cp)
    :inherited("BitRecWnd", pMgr)
    ,_cp(cp)
{
    _pPannel = new CPanel(NULL, this, CPoint(0, 0), CSize(128, 64), 10);
...
void ui::CWifiControWnd::willShow()
{
    ...
    wifi_mode mode = _cp->getWifiMode();
    CAMERALOG("%s() line %d: wifi mode = %d", __FUNCTION__, __LINE__, mode);
...
*   
*/
ui::CLanguageControlWnd::CLanguageControlWnd(CWndManager *pMgr, CCameraProperties* cp)
    :inherited("BitRecWnd", pMgr)
    ,_cp(cp)
    ,_hintCounter(0)
{
    ...
    _pSymble = new CBitLanguageSybl(_pPannel, CPoint(0, 0), CSize(128, 20), true);
    _pSymble->SetStyle
        (BSL_Bit_Black_Pannel
        ,FSL_Bit_White_Text_small
        ,BSL_Bit_Black_Pannel
    ...
    _pRow = new CBlock(_pPannel, CPoint(2, 22), CSize(124, 1), BSL_Bit_White_Pannel);
    _pRow->SetDisabled();

    _pSelector = new CHSelector(_pPannel, NULL, CPoint(0, 26), CSize(128, 20), 1, 5, 128);
    _pSelector->SetStyle(BSL_Bit_Black_Pannel, BSL_Bit_Black_Pannel);

    char** ppList;
    ppList = CLanguageLoader::getInstance()->GetLanguageList(_numLang);
    _pLanguages = new CStaticText*[_numLang];
    CAMERALOG("--- lang num : %d\n", _numLang);
    for (int i = 0; i < _numLang; i++) {
        ...
        _pLanguages[i]->SetVAlign(TOP);
        _pLanguages[i]->SetText((UINT8*)ppList[i]);

        CLanguageLoader::getInstance()->LoadLanguage(ppList[i]);
        memset(_pHintSelect[i], 0x00, maxMenuStrinLen);
        pItem = LoadTxtItem("2nd 3rd key select");
        sprintf(_pHintSelect[i], "%s", pItem->getContent());

        memset(_pHintSet[i], 0x00, maxMenuStrinLen);
        pItem = LoadTxtItem("4th key set");
        sprintf(_pHintSet[i], "%s", pItem->getContent());
    }

    _pHint = new CStaticText(_pPannel, CPoint(0, 48), CSize(128, 16));
    _pHint->SetStyle(BSL_Bit_Black_Pannel
        ,FSL_Bit_White_Text_tiny
        ,BSL_Bit_Black_Pannel
        ,FSL_Bit_White_Text_tiny);
    _pHint->SetHAlign(CENTER);
    _pHint->SetVAlign(MIDDLE);

    _pHint->SetText((UINT8*)_pHintSelect[0]);
}
...
bool ui::CLanguageControlWnd::OnEvent(CEvent *event)
{
    ...
    switch (event->_type)
    {
        case eventType_internal:
            ...
            {
                if ((_hintCounter % 2) == 0) {
                    switch ((_hintCounter / 2) % 2) {
                        case 0:
                            _pHint->SetText((UINT8*)_pHintSelect[(_hintCounter / 2) / 2 % _numLang]);
                            break;
                        case 1:
                            _pHint->SetText((UINT8*)_pHintSet[(_hintCounter / 2) / 2 % _numLang]);
                            break;
                    }
                    _pHint->Draw(false);
                }
        ...
        case eventType_key:
            switch (event->_event)
            {
                // key_up/key_down will be handled by CHSelector
                case key_ok_onKeyUp:
                    {
                        char* language = (char*)_pLanguages[_pSelector->GetCurrentHilight()]->GetText();
                        CAMERALOG("--set language: %s\n", language);
...
void ui::CBitWifiSybl::updateSymble() {
    wifi_mode mode = _cp->getWifiMode();
    switch (mode)
    {
        case wifi_mode_AP:
...
*   
*/
ui::CMicVolumeWnd::CMicVolumeWnd(CWndManager *pMgr, CCameraProperties* cp)
    :inherited("BitRecWnd", pMgr)
    ,_cp(cp)
{
    _pPannel = new CPanel(NULL, this, CPoint(0, 0), CSize(128, 64), 10);
    ...
    _pSymbleTxt = new CBitAudioSybl(_pPannel, CPoint(0, 0), CSize(128, 24),true, _cp);
    _pSymbleTxt->SetStyle
        (BSL_Bit_Black_Pannel
        ,FSL_Bit_White_Text_small
        ,BSL_Bit_Black_Pannel
