...
{
    IPC_AVF_Client::getObject()->RestartRecord();
}
...
int CUpnpCloudCamera::GetWifiCltinfor(char* pName, int len)
{
...
bool CUpnpCloudCamera::setFlightMode(bool enable)
{
    if (enable) {
        // close wifi and bt
        SetWifiMode(2);
        if (CTSDeviceManager::getInstance()) {
            CTSDeviceManager::getInstance()->doEnableBT(false);
        }
    } else {
        // enable wifi and bt
        SetWifiMode(0); // TODO:
...
bool CUpnpCloudCamera::isFlightMode()
{
    EBTStatus st = BTStatus_OFF;
    if (CTSDeviceManager::getInstance()) {
        CTSDeviceManager::getInstance()->getBTStatus();
    }
    bool btOn = true;
    if (st == BTStatus_OFF) {
        btOn = false;
