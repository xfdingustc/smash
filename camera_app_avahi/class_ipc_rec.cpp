...
IPC_AVF_Client::IPC_AVF_Client()
    : CThread("ipc_AVF_client_thread", CThread::NORMAL, 0, NULL)
#ifdef ENABLE_STILL_CAPTURE
    , _bStillMode(false)
    ...
    , _bVideoFullScreen(false)
{
    ...
    avf_media_enable_picture_service(_p_avf_media, "/tmp/mmcblk0p1/DCIM/");
...
IPC_AVF_Client::~IPC_AVF_Client()
{
    ...
    int rval;
    if ((rval = avf_camera_close(_p_avf_media, 1)) < 0) {
...
int IPC_AVF_Client::_configCameraParams(bool bRecord) {
    ...
    avf_media_set_config_bool(_p_avf_media, "config.record", 0, bRecord ? 1 : 0);
...
#endif
    {
        ...
        avf_media_set_config_bool(_p_avf_media, "config.record", 1, bRecord ? 1 : 0);
    }
...
void IPC_AVF_Client::_updateSubtitleConfig()
{
    ...
    if (strcasecmp(tmp, "On") == 0) {
        c = c | 0x01;
        char name[256];
        agcmd_property_get(PropName_camera_name, name , Camera_name_default);
        SetCameraName(name);
        CAMERALOG("set name %s", name);
    }
    ...
    if (strcasecmp(tmp, "中文") == 0) { //chinese
        avf_media_set_config_string(_p_avf_media, "config.subtitle.timefmt", 0,
...
bool IPC_AVF_Client::_setStillCapture() {
    ...
    int rval;
    if (_currentRecOption != media_NULL) {
        rval = avf_camera_stop(_p_avf_media, 0);
        if (rval < 0) {
...
bool IPC_AVF_Client::markVideo() {
    if (_p_avf_media && (_record_state != State_Rec_stoped)) {
        ...
        if (bMarkable) {
            char tmp[256];
            memset(tmp, 0x00, 256);
            agcmd_property_get(PropName_Mark_before, tmp, "15");
            int before_s = atoi(tmp);
            memset(tmp, 0x00, 256);
            agcmd_property_get(PropName_Mark_after, tmp, "15");
...
{
    if ((before_sec < 0) || (after_sec < 0) || !data || (data_size <= 0)) {
        return false;
    }
