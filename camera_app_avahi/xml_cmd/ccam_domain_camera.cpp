...
SCMD_CMD_EXECUTE(AddHost)
{
        ...
        (strcmp(cmd->getPara2(), "") == 0))
    {
        ...
        agcmd_wpa_add_network(wlan_prop, cmd->getPara1(), "open", NULL);
        CAMERALOG("Open SSID");
    } else {
        agcmd_wpa_add(wlan_prop, cmd->getPara1(), "auto", cmd->getPara2());
        // add to buffered list, so that it can be connected immediately:
        agcmd_wpa_add_network(wlan_prop, cmd->getPara1(), "auto", cmd->getPara2());
    }
    savedNumber = agcmd_wpa_list(wlan_prop, AP_List, MAX_AP_NUMBER);
...
SCMD_CMD_EXECUTE(GetUdcPowerDelay)
{
    ...
    memset(delay_sec, 0, sizeof(delay_sec));
...
#ifdef PLATFORMHACHI
    IPC_AVF_Client::getObject()->SetMicMute(bMute);
#else
...
SCMD_CMD_EXECUTE(SetSpeakerStatus)
{
    ...
    //CAMERALOG("SetSpeakerStatus bEnable = %d, volume = %d", bEnable, volume);
    if (volume < 0 || volume > 10) {
        volume = defaultMicGain;
    }
    IPC_AVF_Client::getObject()->SetSpkVolume(volume);

    IPC_AVF_Client::getObject()->SetSpkMute(!bEnable);

    char tmp[] = "1";
    EnumedStringCMD result(CMD_Domain_cam, CMD_Set_Speaker_Status, tmp, NULL);
    StringCMD *pp = &result;
    StringEnvelope ev(&pp, 1);
    if (cmd != NULL) {
        cmd->SendBack(ev.getBuffer(), ev.getBufferLen());
