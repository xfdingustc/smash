    ...
    AVAHI_GCC_UNUSED void* userdata)
{
    ...
    if (record) {
        CAMERALOG("RB: record [%s] on %i.%i is %d",
            t = avahi_record_to_string(record),
            interface, protocol,
            event);
        avahi_free(t);
    } else
        CAMERALOG("RB: [%d]", event);
...
static void sr_callback(
    ...
    if (event != AVAHI_RESOLVER_FOUND)
        CAMERALOG("SR: (%i.%i) <%s> as %s in <%s> [%d]",
        iface, protocol, name, service_type, domain_name, event);
    else {
        ...
        CAMERALOG("SR: (%i.%i) <%s> as %s in <%s>: %s/%s:%i (%s) [%d]",
            iface, protocol, name, service_type, domain_name, hostname, t, port, s, event);
        avahi_free(s);
    }
}
...
void CAvahiServerThread::PrintfServices()
{
    CAMERALOG("######### 1");
sleep(2);
    CAMERALOG("######### 2");
    AvahiKey *k;
    k = avahi_key_new("_ccam._tcp.local", AVAHI_DNS_CLASS_IN, AVAHI_DNS_TYPE_PTR);
    AvahiSRecordBrowser *r = avahi_s_record_browser_new(
        _pserver,
        AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC,
        k, AVAHI_LOOKUP_USE_MULTICAST, record_browser_callback, NULL);
    avahi_key_unref(k);

    AvahiSServiceResolver *sr = avahi_s_service_resolver_new(
        _pserver, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC,
        _pserver_name,
        "_ccam._tcp",
        "local",
